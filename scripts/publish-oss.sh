...
#   OSS_REMOTE=https://github.com/algorisys-oss/nodejs-katas.git ./scripts/publish-oss.sh --push
#
...
OSS_REMOTE="${OSS_REMOTE:-https://github.com/algorisys-oss/nodejs-katas.git}"
...
if [[ "$DRY_RUN" == false && -z "$OSS_REMOTE" ]]; then
    error "OSS_REMOTE is not set. Export it or pass it inline:\n  OSS_REMOTE=https://github.com/algorisys-oss/nodejs-katas.git $0 --push"
fi
